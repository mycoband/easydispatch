...
- Walkthrough capture: when AI Job Walkthrough is on (tech Work + office job), Record video walkthrough is the hero CTA (“Film + narrate — AI writes the report”); video uploads browser→Supabase (not through Vercel body limit). More capture options (voice/photo/library) stays collapsed until expanded — same on mobile and desktop. Tech Extra photos stays secondary/collapsed. If video kind/size fails, run supabase/ai-walkthrough-video.sql.
